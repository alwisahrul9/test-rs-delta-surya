<?php

namespace App\Http\Requests;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class UpdateUserRequest extends FormRequest
{
    public function authorize(): bool
    {
        return true;
    }

    public function rules(): array
    {
        // Ambil ID user dari route
        $user = $this->route('user');
    
        // Dapatkan ID/UUID-nya
        $userId = ($user instanceof \Illuminate\Database\Eloquent\Model) ? $user->id : $user;

        $rules = [
            'name'     => ['required', 'string', 'max:255'],
            // Abaikan email milik sendiri agar tidak dianggap duplikat
            'email'    => ['required', 'email', Rule::unique('users')->ignore($userId)],
            // Password boleh kosong (nullable) jika tidak ingin diubah
            'password' => ['nullable', 'string', 'min:8'],
            'role'     => ['required', Rule::in(['admin', 'doctor', 'pharmacist'])],
            'phone'    => ['required', 'regex:/^08[0-9]{8,11}$/'],
        ];

        // Validasi Kondisional Doctor
        if ($this->input('role') === 'doctor') {
            $rules['str_number']     = ['required', 'string', Rule::unique('doctor_profiles')->ignore($userId, 'user_id')];
            $rules['specialization'] = ['required', 'string'];
            // Signature boleh kosong saat edit jika tidak ingin mengganti gambar lama
            $rules['signature']      = ['nullable', 'image', 'mimes:png,jpg,jpeg', 'max:2048'];
        }

        // Validasi Kondisional Pharmacist
        if ($this->input('role') === 'pharmacist') {
            $rules['sipa_number'] = ['required', 'string', Rule::unique('pharmacist_profiles')->ignore($userId, 'user_id')];
            $rules['work_unit']  = ['required', 'string'];
        }

        return $rules;
    }

    public function messages(): array
    {
        return [
            // Validasi Dasar (Admin, Doctor, Pharmacist)
            'name.required'           => 'Nama wajib diisi.',
            'name.string'             => 'Nama harus berupa teks.',
            'name.max'                => 'Nama maksimal 255 karakter.',
            'email.required'          => 'Email wajib diisi.',
            'email.email'             => 'Format email tidak valid.',
            'email.unique'            => 'Email sudah terdaftar di sistem.',
            'password.min'            => 'Password minimal harus :min karakter.',
            'role.required'           => 'Role wajib dipilih.',
            'role.in'                 => 'Role harus dipilih antara Admin, Doctor, atau Pharmacist (Apoteker).',
            'phone.required'          => 'Nomor HP wajib diisi.',
            'phone.regex'             => 'Format nomor HP harus dimulai dengan 08 dan berjumlah 10-13 digit.',

            // Untuk doctor
            'str_number.required'     => 'Doctor wajib mencantumkan Nomor STR.',
            'str_number.unique'       => 'Nomor STR sudah terdaftar.',
            'specialization.required' => 'Spesialisasi wajib diisi.',
            'signature.required'      => 'Tanda tangan digital wajib diunggah untuk Doctor.',
            'signature.image'         => 'File tanda tangan harus berupa gambar.',
            'signature.mimes'         => 'Format gambar tanda tangan harus PNG, JPG, atau JPEG.',
            'signature.max'           => 'Ukuran gambar tanda tangan maksimal 2MB.',

            // Untuk pharmacist
            'sipa_number.required'    => 'Apoteker wajib mencantumkan Nomor SIPA.',
            'sipa_number.unique'      => 'Nomor SIPA sudah terdaftar.',
            'work_unit.required'     => 'Unit kerja apoteker wajib diisi.',
        ];
    }
}